...
const looks = [
  {
    title: "Quiet Tailoring",
    text: "Clean-cut essentials designed with restraint, structure, and everyday sophistication.",
    image: "/essentials-images/french.png",
    imagePosition: "50% 8%",
  },
  {
    title: "Modern Uniform",
    text: "Refined layers and neutral textures create a wardrobe that feels timeless and relevant.",
    image: "/essentials-images/modern.png",
    imagePosition: "50% 10%",
  },
];
  ...
  return (
    <section className="px-6 pb-20 md:px-10 lg:px-16 xl:px-20 xl:pb-28">
      <div className="grid gap-6 lg:grid-cols-2">
        {looks.map((look) => (
          <article key={look.title} className="group">
            <div className="relative mb-7 overflow-hidden bg-[#111]">
              <div className="relative aspect-[4/5.4] w-full overflow-hidden">
                <Image src={look.image} alt={look.title} fill className="object-cover transition duration-700 ease-out group-hover:scale-[1.015]" style={{ objectPosition: look.imagePosition }} />

                <div className="pointer-events-none absolute inset-0 bg-linear-to-t from-black/12 via-transparent to-black/6" />
              </div>
            </div>
            ...
            <p className="max-w-xl text-[15px] leading-8 text-white/45">{look.text}</p>
          </article>
        ))}
      </div>
    </section>
